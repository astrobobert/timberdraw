...
namespace TimberDraw
{
	...
	public class PostLeft
	{
		...
		public void Draw()
		{
            ...
            {
                StartPoint,
                new Point3d(StartPoint.X + Depth, StartPoint.Y, StartPoint.Z),
                new Point3d(StartPoint.X + Depth, StartPoint.Y + Module1.EaveHt + (Depth * Module1.Pitch), StartPoint.Z),
                new Point3d(StartPoint.X, StartPoint.Y + Module1.EaveHt, StartPoint.Z)
            };
			...
			double tenonZ        = Module1.Make3D ? StartPoint.Z + ((Width - 2) / 2) : 0;
				...
				if (NearJointType == "Tenon")
				{
					...
					// Projects -Y (into sill). LateralDir=(1,0,0) spans Depth in +X.
				...
				// else: Butt or other types -- no geometry, nearParamsJson stays "{}"
				...
				if (FarJointType == "Tenon")
				{
					...
					// Projects +Y (into plate). LateralDir=(1,0,0) spans Depth in +X.
					var farRes = JointFactory.CreateWithPegs(Module1.JointType.Tenon, new JointParams(
					    Module1.JointType.Tenon,
					    new Point3d(StartPoint.X, StartPoint.Y + Module1.EaveHt, tenonZ),
					    new Vector3d(0,  1, 0), new Vector3d(1, 0, 0),
				...
				// else: Butt or other types -- no geometry, farParamsJson stays "{}"
        ...
        private string BuildContextJson()
        {
            ...
            return string.Format(c,
                ...
                ",\"bog\":{5},\"tog\":{6},\"toh\":{7}" +
                ",\"make3D\":{8}" +
                ",\"hasFlrGirt\":{9},\"flrGirtHt\":{10},\"flrGirtDepth\":{11}}}",
                Module1.StartPoint.X, Module1.StartPoint.Y, Module1.StartPoint.Z,
                Module1.EaveHt, Module1.Pitch,
		...
		public void AddMortise(ObjectId JointBlock)
		{
            Module1.AddJoint(TimberId, JointBlock, Module1.Joint.Mortise);
			Module1.DeleteJoint(JointBlock);
		}
	}
}
