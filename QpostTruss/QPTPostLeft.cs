...
namespace TimberDraw
{
	...
	public class QPTPostLeft
	{
		...
		public void Draw()
		{
			...
			pts.Add(StartPoint);
			pts.Add(new Point3d(thirdSpan + Depth, Module1.EaveHt, StartPoint.Z));
			pts.Add(new Point3d(thirdSpan + Depth, Module1.EaveHt + ((thirdSpan - (RafterDepth / Math.Sin(Module1.Beta))) * Module1.Pitch) + (Depth * Module1.Pitch), StartPoint.Z));
			pts.Add(new Point3d(Module1.Span / 3, Module1.EaveHt + ((thirdSpan - (RafterDepth / Math.Sin(Module1.Beta))) * Module1.Pitch), StartPoint.Z));
			...
			if (Module1.HasJoinery) {
				...
				TenonDownId = Module1.DrawElement(pts, 2, "Tenon", "12", "");
                Module1.AddJoint(TimberId, TenonDownId, Module1.Joint.Tenon);
				pts.Clear();
				pts.Add(new Point3d(thirdSpan, Module1.EaveHt + ((thirdSpan - (10 / Math.Sin(Module1.Beta))) * Module1.Pitch), tenonZ));
				...
				TenonUpId = Module1.DrawElement(pts, TenonWidth, "Tenon", "13", "");
        ...
        private string BuildContextJson()
        {
            ...
            return string.Format(c,
                "{{\"class\":\"QPTPostLeft\",\"startX\":{0},\"startY\":{1},\"startZ\":{2},\"rafterDepth\":{3},\"rafterWidth\":{4},\"offsetType\":{5},\"span\":{0},\"eaveHt\":{1},\"pitch\":{2},\"beta\":{3},\"make3D\":{4}}}",
                StartPoint.X, StartPoint.Y, StartPoint.Z, RafterDepth, RafterWidth, Module1.OffsetType, Module1.Span, Module1.EaveHt, Module1.Pitch, Module1.Beta, Module1.Make3D ? "true" : "false");
        }
