...
| `TRelabel` | Retrofit current label conventions onto an older frame (type-first, Dn/Up). `RelabelCommands.cs`. Brace group symbols (*, **: section + angle + finished-stick Overall incl. tenons) re-derive AUTOMATICALLY at every Draw and at every brace EDIT (place/copy/re-seat/re-section/refit/joint cut-clear-delete-sync -- `RelabelBracesIfBrace`, Robert's call 2026-07-15); `TRelabelBraces` = the on-demand run that also echoes the grouping table. |

Joinery commands (each family below; every cutter has a matching `...Del`):

| Command | Connection |
|---|---|
| `TJoint` / `TJointAll` / `TJointDel` | Girt end -> post side: tenon + housing + pegs. `TJointAll` = the DELIBERATE batch: SELECTION-scoped only (no All -- Robert's call; pickfirst honored, selection = who GETS joints), then girt->post / post->sill / summer->girt / joist->carrier / common (ridge housing + eave birdsmouth) / purlin->rafter (housed dovetail, _purlin -- TPurlin's recipe; closes batch-2 #8) passes, each with its own sticky recipe. `TJointDel` (2026-07-15, Robert's call) = the UNIVERSAL delete: any jointed pair, any connection type -- shared id's features stripped BOTH sides + the recipe STAMPS removed (a deliberate delete must not resurrect via TJointSync/replay); two shared joints -> nearest the first pick wins, run again for the other; legacy unkeyed (id 0) girt->post tenon keeps the footprint-overlap fallback. NOTE: the per-family *Del verbs still leave stamps behind (TJointSync can resurrect those) -- TJointDel is the clean delete. |
