...
namespace TimberDraw
{
	...
	public class KPTVertStrutRight
	{
		...
		public void Draw()
		{
			...
			pts.Add(Module1.AtPoint(pts[1], 0, ((strutXLength - Depth) * Module1.Pitch) + 6, 0));
			...
			if (Module1.HasJoinery) {
				...
				PegIt(tenonDownOrigin, (int)Module1.JointEnd.Down);
				pts.Clear();
				pts.Add(new Point3d(Module1.Span - ((((Module1.Span / 2.0) - ((10 / (Math.Sin(Module1.Beta))) + (6 / Module1.Pitch) + 5)) / 2) + ((10 / (Math.Sin(Module1.Beta))) + (6 / Module1.Pitch))), ((((Module1.Span / 2.0) - ((10 / (Math.Sin(Module1.Beta))) + (6 / Module1.Pitch) + 5)) / 2) * Module1.Pitch) + Module1.EaveHt + 6, tenonZ));
				...
				TenonUp = Module1.DrawElement(pts, TenonWidth, "Tenon", "2", "");
		...
		public void PegIt(Point3d Pt, int WhichEnd)
		{
			...
			switch (WhichEnd) {
				...
				case (int)Module1.JointEnd.Down:
					if (Depth < 7)
						Pt = Module1.AtPoint(Pt, 3, -1.75, -0.75);
					else
						Pt = Module1.AtPoint(Pt, 4, -1.75, -0.75);
					PegCol.Add(Module1.DrawPeg(Pt, r, Width + 1.5, "Peg", "", "", ""));
					break;
			}
        ...
        private string BuildContextJson()
        {
            ...
            return string.Format(c,
                "{{\"class\":\"KPTVertStrutRight\",\"startX\":{0},\"startY\":{1},\"kPostWidth\":{0},\"kPostDepth\":{1},\"kpRafterDepth\":{2},\"postWidth\":{3},\"span\":{4},\"eaveHt\":{5},\"pitch\":{6},\"beta\":{7},\"make3D\":{8},\"offsetType\":{9}}}",
