	...
	public class KPTRafterLeft
	{
		...
		public void Draw()
		{
			Point3dCollection pts = new()
            {
                ...
                new Point3d(((Module1.Span / 2) - (KPostDepth / 2)) + (Math.Sin(Module1.Beta) * 3), (((Module1.Span / 2) - ((Depth / (Math.Sin(Module1.Beta))) + ((KPostDepth / 2) - (Math.Sin(Module1.Beta) * 3)))) * Module1.Pitch) + Module1.EaveHt, 0),
                new Point3d((Module1.Span / 2) - (KPostDepth / 2), (((Module1.Span / 2) - (((Depth - 3) / (Math.Sin(Module1.Beta))) + (KPostDepth / 2))) * Module1.Pitch) + Module1.EaveHt, 0),
                new Point3d((Module1.Span / 2) - (KPostDepth / 2), Module1.EaveHt + (((Module1.Span / 2) - (KPostDepth / 2)) * Module1.Pitch), 0)
            };
			...
			if (Module1.HasJoinery) {
				double tenonZ = 0;
				if (Module1.Make3D)
					tenonZ = StartPoint.Z + ((Width - TenonWidth) / 2);
				else
					tenonZ = 0;
				pts.Clear();
				...
				Tenon = Module1.DrawElement(pts, TenonWidth, "Tenon", "9", "");
        ...
        private string BuildContextJson()
        {
            ...
            return string.Format(c,
                "{{\"class\":\"KPTRafterLeft\",\"startX\":{0},\"startY\":{1},\"startZ\":{2},\"kPostDepth\":{3},\"postDepth\":{4},\"kpostRafterSitDepth\":{5},\"span\":{0},\"eaveHt\":{1},\"pitch\":{2},\"beta\":{3},\"make3D\":{4}}}",
