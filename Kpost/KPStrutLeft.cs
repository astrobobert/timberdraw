...
namespace TimberDraw
{
	public class KPStrutLeft
	{
		...
		public ObjectId TenonDownId;  // far end:  king post left face
		...
		// Pts captured during Draw() for use by the orchestrator (KPBent) when registering
		// the oblique rafter-contact edge in BentNetwork as a JointParams.ForPolygon entry.
		...
		public void Draw()
		{
			...
			pts.Add(new Point3d((((halfSpan - (postDepth + (KPostDepth / 2))) / 2) + postDepth) + ((Depth / Math.Sin(Math.Atan(Module1.Pitch)) / 2)), (((halfSpan - (postDepth + (KPostDepth / 2))) / 2) * Module1.Pitch) + Module1.TOH + ((Depth / Math.Cos(Math.Atan(Module1.Pitch)) / 2)), _StartPoint.Z));
			...
			if (NearJointType == "Tenon") {
				...
				var np1 = Module1.PolarPoint(np0, Module1.Beta, Depth / Math.Cos((Math.PI / 2) - (Module1.Beta * 2)));
				var np2 = Module1.PolarPoint(np1, Module1.Beta + (Math.PI / 2) + ((Math.PI / 2) - (Module1.Beta * 2)), 4 / Math.Cos((Math.PI / 2) - (Module1.Beta * 2)));
				var np3 = Module1.PolarPoint(np0, Module1.Beta + (Math.PI / 2), 4);
				pts.Add(np0); pts.Add(np1); pts.Add(np2); pts.Add(np3);
				NearTenonPts   = new[] { np0, np1, np2, np3 };
				NearTenonWidth = nearTW;
				TenonUpId = Module1.DrawElement(pts, nearTW, "Tenon", "5", "");
			...
			// Far (Down) tenon: oblique face at king post left face.
			if (FarJointType == "Tenon") {
				...
				TenonDownId = Module1.DrawElement(pts, farTW, "Tenon", "6", "");
            ...
            // End markers: "N" at near (rafter contact) face, "F" at far (king post left face).
