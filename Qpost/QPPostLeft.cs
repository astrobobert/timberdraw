...
namespace TimberDraw
{
	public class QPPostLeft
	{
		...
		public Point3d[]   FarTenonPts;   // oblique rafter-contact polygon pts for BentNetwork
		...
		public void Draw()
		{
            ...
            {
                StartPoint,
                new Point3d(thirdSpan + Depth, Module1.TOG, StartPoint.Z),
                new Point3d(thirdSpan + Depth, Module1.TOH + ((thirdSpan - postDepth) * Module1.Pitch) + (Depth * Module1.Pitch), StartPoint.Z),
                new Point3d(thirdSpan, Module1.TOH + ((thirdSpan - postDepth) * Module1.Pitch), StartPoint.Z)
            };
			...
			// Far (Up) rafter-contact tenon: oblique face, keep DrawElement geometry.
			{
				...
				pts.Clear();
				pts.Add(new Point3d(thirdSpan, Module1.TOH + ((thirdSpan - postDepth) * Module1.Pitch), tenonZ));
				...
				TenonUpId = Module1.DrawElement(pts, farTW, "Tenon", "13", "");
				Module1.AddJoint(TimberId, TenonUpId, Module1.Joint.Tenon);
				FarTenonPts  = new Point3d[] { pts[0], pts[1], pts[2], pts[3] };
				FarTenonWidth = farTW;
				// Peg for far (Up) end:
				{
					double angle = Depth < 7 ? 1.0427 + Math.Atan(Module1.Pitch) + 1.5708
					                         : 1.15839 + Math.Atan(Module1.Pitch) + 1.5708;
					double offset = Depth < 7 ? 3.47312 : 4.36607;
					Point3d pegPt = Module1.PolarPoint(pts[1], angle, offset);
        ...
        private string BuildContextJson()
        {
            ...
            return string.Format(c,
                "{{\"class\":\"QPPostLeft\",\"startX\":{0},\"startY\":{1},\"girtWidth\":{2},\"postDepth\":{3},\"span\":{4},\"make3D\":{5},\"offsetType\":{6},\"tog\":{7},\"toh\":{8},\"pitch\":{9},\"beta\":{10}}}",
