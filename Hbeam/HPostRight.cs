	...
	public class HPostRight
	{
		...
		public void Draw()
		{
			...
			if (Module1.HasJoinery) {
				...
				if (Module1.Make3D)
					...
					tenonZ = 0;
				// Near-end tenon (at foot): Origin at StartPoint, projects -Y (into plate), spans +X (Depth toward right).
				Point3d tenonDownOrigin = new Point3d(StartPoint.X, StartPoint.Y, tenonZ);
				var nearP = new JointParams(Module1.JointType.Tenon,
                    tenonDownOrigin,
                    new Vector3d(0, -1, 0),  // FaceNormal: into HBeamRight below
                    new Vector3d(1, 0, 0),   // LateralDir: along Depth (+X, toward right)
				...
				pts.Clear();
				pts.Add(Module1.AtPoint(new Point3d(StartPoint.X, StartPoint.Y, tenonZ), 0, hpLengthLong, 0));
				pts.Add(Module1.AtPoint(pts[0], Depth, -(Depth * Module1.Pitch), 0));
				pts.Add(Module1.PolarPoint(pts[1], (Math.PI / 2), 4 / Math.Cos(Module1.Beta)));
				pts.Add(Module1.PolarPoint(pts[0], Module1.rad(90) - Module1.Beta, 4));
        ...
        private string BuildContextJson()
        {
            ...
            return string.Format(c,
                "{{\"class\":\"HPostRight\",\"startX\":{0},\"startY\":{1},\"startZ\":{2},\"postDepth\":{3},\"postWidth\":{4},\"kpostDepth\":{5},\"rafterWidth\":{6},\"hbDivisor\":{7},\"span\":{8},\"pitch\":{9},\"beta\":{10},\"make3D\":{11}}}",
