...
namespace TimberDraw
{
	...
	public class HBeamRight
	{
		...
		public JointParams FarJointParamsDrawn;   // params used to draw Tenon (into right post)
		...
		public void Draw()
		{
			...
			pts.Add(Module1.PolarPoint(pts[1], Module1.rad(180), hbLength + 4));
			pts.Add(Module1.PolarPoint(pts[0], Module1.rad(180), hbLength + 4));
			...
			if (Module1.HasJoinery) {
				...
				if (Module1.Make3D)
					...
					tenonZ = 0;
				// Tenon at right-post end: Origin at StartPoint, projects +X (into right wall post), spans +Y (Depth).
				Point3d tenonOrigin = new Point3d(StartPoint.X, StartPoint.Y, tenonZ);
				var farP = new JointParams(Module1.JointType.Tenon,
                    tenonOrigin,
                    new Vector3d(1, 0, 0),   // FaceNormal: into right wall post
            ...
            // End markers: "N" at near (inner/center Butt) face, "F" at far (right-post Tenon) face.
        ...
        private string BuildContextJson()
        {
            ...
            return string.Format(c,
                "{{\"class\":\"HBeamRight\",\"startX\":{0},\"startY\":{1},\"startZ\":{2},\"hbLength\":{3},\"postWidth\":{4},\"kpostDepth\":{5},\"hbDivisor\":{6},\"make3D\":{7}}}",
