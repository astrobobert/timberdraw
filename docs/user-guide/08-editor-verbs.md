...
| `TFit` | a timber's **end**, then a target face | Trims or extends that end onto the face (square or mitered); the other end stays put. Editing a skeleton member **pins** it (survives re-Generate; stops following the recipe). |
| `TSection` | a timber | Re-sections it (new W x D) in place. Pins a skeleton member. |
| `TScarf` | a timber + a point | Splits it into two pieces with a scarf, and remembers the splice interface. The pieces keep the timber's identity; a skeleton member's pieces are pinned. |
