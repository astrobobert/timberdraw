...
| **TJointAll cut fewer joints than expected** | Contacts already carrying a joint are skipped (idempotent), and only end-into-side bearings qualify (girt->post, post->sill, summer->girt). | The echo reports cut / skipped / failed. `TScan` to inspect what actually touches what. |
