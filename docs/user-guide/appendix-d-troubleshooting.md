...
| **TJointAll cut fewer joints than expected** | Contacts already carrying a joint are skipped (idempotent), only end-into-side bearings qualify (girt->post, post->sill, summer->girt, joist->carrier), and a Select scope only joints the SELECTED timbers. | The echo reports cut / skipped / failed per pass. `TScan` to inspect what actually touches what. |
...
| **A moved timber's joints are in the old place / a re-Generated girt lost its pockets** | Joinery travels with the timber; the mate's half doesn't move with it, and a regenerated skeleton member comes back uncut. | Select the moved/surviving timbers and run `TJointSync` (Ch. 10.3): it re-cuts by stored recipe, re-attaching to replaced members. |
